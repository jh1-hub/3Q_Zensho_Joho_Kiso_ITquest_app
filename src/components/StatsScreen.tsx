...
export default function StatsScreen({
  ...
  const handlePrecacheAll = async () => {
    ...
    const urls = [
      ...
      '/img/monsters/it_slime_thumb.jpg',
      '/img/monsters/it_slime_battle.jpg',
      '/img/monsters/bug_dracky_thumb.jpg',
      '/img/monsters/bug_dracky_battle.jpg',
      '/img/monsters/compile_cat_thumb.jpg',
      '/img/monsters/compile_cat_battle.jpg',
      '/img/monsters/router_thumb.jpg',
      '/img/monsters/router_battle.jpg',
      '/img/monsters/hacker_man_thumb.jpg',
      '/img/monsters/hacker_man_battle.jpg',
      '/img/monsters/lan_cable_thumb.jpg',
      '/img/monsters/lan_cable_battle.jpg',
      '/img/monsters/packet_thumb.jpg',
      '/img/monsters/packet_battle.jpg',
      '/img/monsters/noise_shroom_thumb.jpg',
      '/img/monsters/noise_shroom_battle.jpg',
      '/img/monsters/killer_hdd_thumb.jpg',
      '/img/monsters/killer_hdd_battle.jpg',
      '/img/monsters/stray_port_thumb.jpg',
      '/img/monsters/stray_port_battle.jpg',
      '/img/monsters/scorpion_thumb.jpg',
      '/img/monsters/scorpion_battle.jpg',
      '/img/monsters/cookie_mud_thumb.jpg',
      '/img/monsters/cookie_mud_battle.jpg',
      '/img/monsters/memory_bomb_thumb.jpg',
      '/img/monsters/memory_bomb_battle.jpg',
      '/img/monsters/virus_golem_thumb.jpg',
      '/img/monsters/virus_golem_battle.jpg',
      '/img/monsters/security_satan_thumb.jpg',
      '/img/monsters/security_satan_battle.jpg',
      '/img/monsters/zombie_server_thumb.jpg',
      '/img/monsters/zombie_server_battle.jpg',
      '/img/monsters/logic_gate_thumb.jpg',
      '/img/monsters/logic_gate_battle.jpg',
      '/img/monsters/ransomware_beast_thumb.jpg',
      '/img/monsters/ransomware_beast_battle.jpg',
      '/img/monsters/gigadeath_db_thumb.jpg',
      '/img/monsters/gigadeath_db_battle.jpg',
      '/img/monsters/code_master_thumb.jpg',
      '/img/monsters/code_master_battle.jpg'
    ];
    ...
    try {
      ...
      const cache = await caches.open('it-quest-cache-v2');
      let completed = 0;
      ...
      for (const url of urls) {
        ...
        completed++;
        setPrecacheProgress(Math.round((completed / urls.length) * 100));
      }
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-sky-300 via-sky-101 to-emerald-100 text-slate-800 p-4 md:p-6 flex flex-col font-sans select-none relative overflow-hidden border-t-8 border-blue-600">
      ...
      <div className="max-w-6xl w-full mx-auto flex flex-col gap-6 flex-1 z-10">
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4">
          ...
          <div className="bg-white/95 p-4 rounded-xl border-2 border-amber-150 shadow-md relative overflow-hidden flex flex-col justify-between">
            ...
            <div className="mt-2 flex items-baseline">
              {bestTime ? (
                <span className="text-xl md:text-2xl font-black text-amber-900 truncate max-w-full">
                  {formatTime(bestTime)}
                </span>
              ) : (
                <span className="text-lg font-black text-slate-400">記録なし</span>
              )}
            </div>
            <p className="text-[9px] text-slate-400 mt-2 font-semibold">ペナルティ秒を含む最速秒数</p>
          </div>

          {/* 4. 平均正答率 */}
          <div className="bg-white/95 p-4 rounded-xl border-2 border-purple-150 shadow-md relative overflow-hidden flex flex-col justify-between">
            <div className="absolute top-2 right-2 p-1.5 bg-purple-50 text-purple-600 rounded-full">
              <TrendingUp size={18} />
            </div>
            <span className="text-[10px] text-slate-400 font-black uppercase tracking-wider block font-bold">総合 正答率</span>
            <div className="mt-2 flex items-baseline gap-1.5">
              <span className="text-3xl font-black text-purple-950">{overallAccuracy}%</span>
              <span className="text-xs font-bold text-slate-500">
                ({totalQuizCorrects}/{totalQuizAttempts}問)
              </span>
            </div>
            <p className="text-[9px] text-slate-400 mt-2 font-semibold">全問題に対する正解割合</p>
          </div>

          {/* 5. 時の回廊ハイスコア */}
          <div className="bg-white/95 p-4 rounded-xl border-2 border-indigo-150 shadow-md relative overflow-hidden flex flex-col justify-between">
            <div className="absolute top-2 right-2 p-1.5 bg-indigo-50 text-indigo-600 rounded-full">
              <Timer size={18} className="animate-spin text-indigo-500" style={{ animationDuration: '6s' }} />
            </div>
            <span className="text-[10px] text-slate-400 font-black uppercase tracking-wider block font-bold">回廊ハイスコア</span>
            <div className="mt-2 flex items-baseline gap-1.5 font-mono">
              <span className="text-2xl font-black text-indigo-950">{gameStats.timeAttackHighScore || 0}</span>
              <span className="text-xs text-slate-550 font-bold font-sans">pts</span>
            </div>
            <p className="text-[9px] text-slate-400 mt-2 font-semibold">ときのかいろうの最高得点</p>
          </div>
        ...
        {/* PWA プリキャッシュ管理（通信節約）カード */}
        <div className="bg-gradient-to-r from-blue-900 to-indigo-950 p-5 rounded-2xl border-2 border-cyan-400 text-white shadow-lg relative overflow-hidden">
          <div className="absolute top-0 right-0 p-8 opacity-10 pointer-events-none">
            <Download size={120} />
          </div>
          <div className="relative z-10 flex flex-col md:flex-row md:items-center justify-between gap-4">
            <div className="space-y-1.5 max-w-2xl">
              <span className="bg-cyan-500/20 text-cyan-300 text-[9px] font-black tracking-widest px-2 py-0.5 rounded border border-cyan-400/30 uppercase">
                PWA DATA-SAVING SYSTEM
              </span>
              <h3 className="text-base font-black text-cyan-200 tracking-wider flex items-center gap-1.5">
                <span>📡 ギガ節約・全ゲーム画像の一括ローカル保存</span>
              </h3>
              <p className="text-[11px] text-slate-350 leading-relaxed font-medium">
                ゲームで使用するすべてのモンスターイラストや魔導士画像を、あなたのお使いのデバイスに一括ダウンロードして保存します（PWAキャッシュ機能）。これにより、<strong>今後のゲームプレイ中の通信量をゼロ</strong>に抑え、オフライン地域や通信制限下でも一瞬で高画質にバトルが読み込めます！
              </p>
            </div>
